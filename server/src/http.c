...
int check_file_permissions(char* path) {
    if (access(path, R_OK) != 0) {
        if (errno == EACCES) {
            LOG_ERROR("Permission denied to read file: %s", path);
            return STATUS_ERROR_FORBIDDEN;
        } else if (errno == ENOENT) {
            return STATUS_ERROR_NOT_FOUND;
        } else {
            return STATUS_ERROR_INTERNAL_SERVER_ERROR;
        }
    }
...
void send_response(char* path, int clientfd, http_method_t method) {
    LOG_INFO("check_file_permissions");
    int permission_status = check_file_permissions(path);
    if (permission_status != STATUS_SUCCESS) {
        send_status(clientfd, permission_status);
        return;
    }
